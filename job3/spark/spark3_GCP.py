...
conf = SparkConf().setMaster("yarn").setAppName("Job3")
...
hsp = sqlContext \
	  ...
	  .load("gs://bug-data/input/historical_stock_prices/historical_stock_prices.csv") \
	  .rdd
...
hs = sqlContext \
	 ...
	 .options(header='true', inferschema='true', quote='"', delimiter=',') \
	 .load("gs://bug-data/input/historical_stocks/historical_stocks.csv") \
	 .rdd
